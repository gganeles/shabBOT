...
async function timeTick(client, allEvents, soccerDebug) {
    const date = new Date()
...
async function sendTovaTrigger(now,client) {
    if (tovaBool) {
        if (now - prevTovaTime > tovaInterval) {
            client.sendMessage(tovaChat,'Dearest Tova,\nI hope this message finds you well. This is your dearest nephew Gabe, and I just wanted to wish you an amazing hour. Also, you should drink water.')
            prevTovaTime = now;
        }
    }
}

function tovaTriggerStart(prompt) {
    tovaBool = true
    prevTovaTime = new Date()
    const promptlist = prompt.split(/\s+/)
    if (promptlist.length == 1) {
        tovaInterval = 60*60*1000
    } else {
        tovaInterval = parseInt(promptlist[1])*60*1000
    }
    return "Water Messages Activated With Interval of "+tovaInterval/60000+" minutes"
}

function tovaTriggerEnd() {
    tovaBool = false
    return "Water Messages Deactivated"
}
...
module.exports = { timeTick, newSoccerEvent, tovaTriggerEnd, tovaTriggerStart }
