...
const { rename, remove, settime } = require('../eventstuff/EventEditCmds.js');
const { coming, leave } = require('../eventstuff/updateAttendeeCommands.js');
...
const { shopCMD, shopListCMD, unshopCMD } = require('../utilitystuff/shopping.js');
    ...
    try {
        for (let q = 0; q < promptList.length; q++) {
            if (/*msg.mentionedIds.includes(client.info.wid._serialized) || this is for checking when tagged */ msg.body.split('\n').at(q).startsWith("!")) {
                ...
                } else if (prompt.match(/^(reminders|rems|todo)\b/gi)) {
                ...
                } else if (/^(shop|shp|sh)\b/i.test(prompt)) {
                    msg.reply(shopCMD(prompt, allEvents[chat]));
                } else if (/^(unshop|unshp|unsh)\b/i.test(prompt)) {
                    msg.reply(unshopCMD(prompt, allEvents[chat]));
                } else if (/^(shoplist|shplist|shoppinglist|shlst|shlist|shls)\b/i.test(prompt)) {
                    msg.reply(shopListCMD(allEvents[chat]));
                }
            }
        }
    } catch (error) {
